...
[![newSingleThreadExecutor01.png](https://z3.ax1x.com/2021/08/06/fn2zee.png)](https://imgtu.com/i/fn2zee)

执行上述代码结果如下，可以看出所有的 task 始终是在同一个线程中被执行的。

[![newSingleThreadExecutor02.png](https://z3.ax1x.com/2021/08/06/fnRVOS.png)](https://imgtu.com/i/fnRVOS)

#### newCachedThreadPool

创建一个可缓存线程池，如果线程池长度超过处理需要，可灵活回收空闲线程，若无可回收，则新建线程。

[![newCachedThreadPool01.png](https://z3.ax1x.com/2021/08/06/fnR7nS.png)](https://imgtu.com/i/fnR7nS)

执行效果如下：

[![newCachedThreadPool02.png](https://z3.ax1x.com/2021/08/06/fnRLkj.png)](https://imgtu.com/i/fnRLkj)

从上面日志中可以看出，缓存线程池会创建新的线程来执行任务。但是如果将代码修改一下，在提交任务之前休眠 1 秒钟，如下：

[![newCachedThreadPool03.png](https://z3.ax1x.com/2021/08/06/fnWSXT.png)](https://imgtu.com/i/fnWSXT)
...
[![newFixedThreadPool01.png](https://z3.ax1x.com/2021/08/06/fn4KBT.png)](https://imgtu.com/i/fn4KBT)

上述代码创建了一个固定数量 3 的线程池，因此虽然向线程池提交了 10 个任务，但是这 10 个任务只会被 3 个线程分配执行，执行效果如下：

[![newFixedThreadPool02.png](https://z3.ax1x.com/2021/08/06/fn4y8A.png)](https://imgtu.com/i/fn4y8A)

#### newScheduledThreadPool

创建一个定时线程池，支持定时及周期性任务执行。

[![newScheduledThreadPool01.png](https://z3.ax1x.com/2021/08/06/fn5uRA.png)](https://imgtu.com/i/fn5uRA)

上面代码创建了一个线程数量为 2 的定时任务线程池，通过 scheduleAtFixedRate 方法，指定每隔 500 毫秒执行一次任务，并且在 5 秒钟之后通过 shutdown 方法关闭定时任务。执行效果如下：

[![newScheduledThreadPool02.png](https://z3.ax1x.com/2021/08/06/fnIGOx.png)](https://imgtu.com/i/fnIGOx)
...
[![现实案例01.gif](https://z3.ax1x.com/2021/08/06/fn7HBt.gif)](https://imgtu.com/i/fn7HBt)

如果订单量突然大幅增加，3 台机器已经处于满负荷状态，一时间无法完成新增的订单任务。那怎么办呢？正所谓钱是不可能不挣的，只能硬着头皮接下新来的订单，但是会将新来的订单暂存在仓库中，当有加工机器空闲下来之后，再用来生产仓库中的订单，如下图所示：

[![现实案例02.gif](https://z3.ax1x.com/2021/08/06/fnHcvj.gif)](https://imgtu.com/i/fnHcvj)

如果订单量持续快速增长，导致仓库也存储满了。那怎么办呢？ 正常情况下加工厂肯定会通过购买新的加工机器来满足订单需求，如下所示：

[![现实案例03.png](https://z3.ax1x.com/2021/08/06/fnHoPU.png)](https://imgtu.com/i/fnHoPU)

有了仓库和新购买的加工机器加持，加工厂业务还是能够正常流转。但是当某些极端情况发生，比如双十一搞活动之后订单爆单了。这时新增的订单任务连仓库以及所有的加工机器都已经无法容纳，说明加工厂已经不能接受新的订单任务了，因此只能拒绝所有新的订单。

[![现实案例04.png](https://z3.ax1x.com/2021/08/06/fnHOq1.png)](https://imgtu.com/i/fnHOq1)

线程池的工作流程同上面描述的加工厂完成订单任务非常相似，并且在线程池的构造器中，通过传入的参数可以设置默认有多少台加工机器、仓库的大小、可以购买新的加工机器的最大数量等等。

#### 线程池结构

[![线程池结构01.png](https://z3.ax1x.com/2021/08/06/fnbtiT.png)](https://imgtu.com/i/fnbtiT)

从上图中可以大体看出，在线程池内部主要包含以下几个部分：

worker 集合：保存所有的核心线程和非核心线程（类比加工厂的加工机器），其本质是一个HashSet。

[![线程池结构02.png](https://z3.ax1x.com/2021/08/06/fnqIc4.png)](https://imgtu.com/i/fnqIc4)

等待任务队列：当核心线程的个数达到 corePoolSize 时，新提交的任务会被先保存在等待队列中（类比加工厂中的仓库），其本质是一个阻塞队列 BlockingQueue。

[![线程池结构03.png](https://z3.ax1x.com/2021/08/06/fnqx3D.png)](https://imgtu.com/i/fnqx3D)

ctl：是一个 AtomicInteger 类型，二进制高 3 位用来标识线程池的状态，低 29 位用来记录池中线程的数量。

获取线程池状态、工作线程数量、修改 ctl 的方法分别如下：

[![线程池结构04.png](https://z3.ax1x.com/2021/08/06/fnLtv4.png)](https://imgtu.com/i/fnLtv4)

线程池主要有以下几种运行状态：

[![线程池结构05.png](https://z3.ax1x.com/2021/08/06/fnOqfK.png)](https://imgtu.com/i/fnOqfK)

#### 参数分析

线程池的构造器如下：

[![参数分析01.png](https://z3.ax1x.com/2021/08/06/fnXN7R.png)](https://imgtu.com/i/fnXN7R)
...
[![流程解析01.png](https://z3.ax1x.com/2021/08/06/fnvvXF.png)](https://imgtu.com/i/fnvvXF)

上面代码创建了 3 个固定数量的线程池，每次提交的任务耗时 100 毫秒。每次提交任务之前都会延迟2秒，保证线程池中的工作线程都已经执行完毕，但是执行效果如下：

[![流程解析02.png](https://z3.ax1x.com/2021/08/06/fnxX4I.png)](https://imgtu.com/i/fnxX4I)
...
[![流程解析03.png](https://z3.ax1x.com/2021/08/06/fnzHzV.png)](https://imgtu.com/i/fnzHzV)

上述代码提交的任务耗时 4 秒，因此前 2 个任务会占用线程池中的 2 个核心线程。此时有新的任务提交给线程池时，任务会被缓存到等待队列中，结果如下：

[![流程解析04.png](https://z3.ax1x.com/2021/08/06/fuSiQK.png)](https://imgtu.com/i/fuSiQK)
...
[![流程解析05.png](https://z3.ax1x.com/2021/08/06/fuSGwQ.png)](https://imgtu.com/i/fuSGwQ)

上述代码创建了一个核心线程数为 2，最大线程数为 10，等待队列长度为 2 的线程池。执行效果如下：

[![流程解析06.png](https://z3.ax1x.com/2021/08/06/fuSTTH.png)](https://imgtu.com/i/fuSTTH)
...
[![流程解析07.png](https://z3.ax1x.com/2021/08/06/fuCMS1.png)](https://imgtu.com/i/fuCMS1)

修改最大线程数为 3，并提交 6 次任务给线程池，执行效果如下：

[![流程解析08.png](https://z3.ax1x.com/2021/08/06/fuCflq.png)](https://imgtu.com/i/fuCflq)

程序会报异常 RejectedExecutionException，拒绝策略是线程池的一种保护机制，目的就是当这种无节制的线程资源申请发生时，拒绝新的任务保护线程池。默认拒绝策略会直接报异常，但是 JDK 中一共提供了 4 种保护策略，如下：

[![流程解析09.png](https://z3.ax1x.com/2021/08/06/fuCbtJ.png)](https://imgtu.com/i/fuCbtJ)
...
[![为何禁止使用 Executors01.png](https://z3.ax1x.com/2021/08/06/fuiUMt.png)](https://imgtu.com/i/fuiUMt)

上述代码创建了一个固定数量为 2 的线程池，并通过 for 循环向线程池中提交 100 万个任务。
通过 java -Xms4m -Xmx4m FixedThreadPoolOOM 执行上述代码：

[![为何禁止使用 Executors02.png](https://z3.ax1x.com/2021/08/06/fuiDIg.png)](https://imgtu.com/i/fuiDIg)

可以发现当任务添加到 7 万多个时，程序发生 OOM。这是为什么呢？ 看一下newSingleThreadExecutor 和 newFixedThreadPool() 的具体实现，如下：

[![为何禁止使用 Executors03.png](https://z3.ax1x.com/2021/08/06/fui5oF.png)](https://imgtu.com/i/fui5oF)
...
[![为何禁止使用 Executors04.png](https://z3.ax1x.com/2021/08/06/fui7W9.png)](https://imgtu.com/i/fui7W9)

上述代码同样会报 OOM，只是错误的 log 信息有点区别：无法创建新的线程。

[![为何禁止使用 Executors05.png](https://z3.ax1x.com/2021/08/06/fuiLsx.png)](https://imgtu.com/i/fuiLsx)

看一下 newCachedThreadPool 的实现：

[![为何禁止使用 Executors06.png](https://z3.ax1x.com/2021/08/06/fuFFyt.png)](https://imgtu.com/i/fuFFyt)
